"""
Self-play training for Monte Carlo agent.
Trains agent by playing against itself with episodic updates.
"""
...
def play_game(agent1, agent2, env: TicTacToeEnv, training_mode: bool = True) -> Tuple[int, List]:
    ...
    while not env.is_terminal():
        ...
        if training_mode and hasattr(current_agent, 'update_episode'):
...
def train_monte_carlo_agent(num_episodes: int = 10000, eval_interval: int = 1000) -> MonteCarloAgent:
    """
    Train Monte Carlo agent through self-play.
    ...
    # Create agents
    agent1 = MonteCarloAgent(player_id=1, epsilon=0.1)
    agent2 = MonteCarloAgent(player_id=2, epsilon=0.1)
    env = TicTacToeEnv()
    ...
    for episode in range(1, num_episodes + 1):
        # Self-play training
        winner, game_history = play_game(agent1, agent2, env, training_mode=True)
        
        # Update agents with episode history
        if game_history:
            agent1.update_episode(game_history)
            agent2.update_episode(game_history)
    ...
    if win_rates:
        ...
        plt.grid(True)
        plt.savefig('monte_carlo_training.png')
        plt.close()
...
if __name__ == "__main__":
    # Train the agent
    trained_agent = train_monte_carlo_agent(num_episodes=20000, eval_interval=1000)
    
    # Save the trained agent
    trained_agent.save('trained_mc_agent.pkl')
    print("Trained agent saved to 'trained_mc_agent.pkl'") 